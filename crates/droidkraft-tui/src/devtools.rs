...
mod tests {
    ...
    fn test_find_gradle_root_walks_up() {
        ...
        let tmp = std::env::temp_dir().join("droidkraft_test_gradle");
    ...
    fn test_find_gradle_root_not_found() {
        ...
        let (found, has) =
            find_gradle_root(std::path::Path::new("/tmp/nonexistent_droidkraft_xyz"));
        assert!(!has);
    ...
    fn test_discover_app_modules_with_app_plugin() {
        let tmp = std::env::temp_dir().join("droidkraft_test_app_modules");
